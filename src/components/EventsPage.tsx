...
const EventTree: React.FC<EventTreeProps> = ({
  ...
  const handleEventClick = (event: Event) => {
    ...
    navigate(`/event/${formattedTitle}`); // Fixed template literal
  };
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      <div
        ...
        style={{ backgroundImage: `url(${backgroundImageUrl})` }} // Fixed template literal
      />
      <div className="absolute inset-0 bg-blue-900 bg-opacity-10" />
      <div className="absolute inset-0 overflow-hidden">
        {Array.from({ length: 15 }).map((_, i) => (
          <div
            key={`particle-${i}`} // Fixed template literal
            className="absolute rounded-full bg-yellow-100 bg-opacity-30"
            style={{
              width: `${Math.random() * 4 + 1}px`, // Fixed template literals
              height: `${Math.random() * 4 + 1}px`,
              top: `${Math.random() * 100}%`,
              left: `${Math.random() * 100}%`,
              boxShadow: "0 0 5px 1px rgba(255, 255, 255, 0.2)",
              animation: `float ${Math.random() * 20 + 10}s linear infinite`, // Fixed template literal
      ...
      {arrangedEvents.map((event, index) => {
        ...
        return (
          <div key={`node-${index}`} className="group"> {/* Fixed template literal */}
            <div
              style={nodeStyle}
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  border: getCategoryColor(event.category).pingBorder, // Fixed to use the function instead of nodeStyle
                }}
              ></div>
            </div>
            ...
            <div
              ...
              style={{
                top: `calc(${position.top} + 40px)`, // Fixed template literal
                left: position.left,
